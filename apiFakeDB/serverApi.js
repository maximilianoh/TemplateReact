...
const whatShow = (pathShow, data, params) => {
  if (pathShow === '/hello/test') {
    return { datos: params.text };
  }
  return data;
};
  ...
  lista.forEach((p) => {
    if (p.path !== undefined && p.method !== undefined) {
      app[p.method.toLowerCase()](p.path, (req, res) => {
        const valueReturn = whatShow(p.path, p.datos, req.body);
...
reload(app).then(() => {
  ...
  console.error('Reload could not start, could not start server/sample app', err); // eslint-disable-line no-console
});
